...
def compute_angle(transform):
    ...
    return np.arccos(min(1, max(-1, (np.trace(transform[0:3, 0:3])
                                     - 1)/2)))*180.0/np.pi
...
def dcm2rpy(matrix):
    ...
    if np.abs(rpy[1] - math.pi/2.0) < 0.00001:
        ...
        if np.abs(rpy[1] + math.pi/2.0) < 0.00001:
            ...
            rpy[2] = math.atan2(R[1, 0]/math.cos(rpy[1]),
                                R[0, 0]/math.cos(rpy[1]))
            rpy[0] = math.atan2(R[2, 1]/math.cos(rpy[1]),
                                R[2, 2]/math.cos(rpy[1]))
