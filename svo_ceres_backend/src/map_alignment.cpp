...
MapAlignmentSE3::MapAlignmentSE3(const MapAlignmentOptions& map_alignment_options) :
    num_points_(0), max_num_points_(100),
    options_(map_alignment_options)
{
...
void MapAlignmentSE3::addCorrespondencePair(const Position &old_pos, const Position &new_pos)
{
...
bool MapAlignmentSE3::getTransformation(Transformation &T_old_new)
{
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    /* Don't close loop if associated points are too far apart. This can happen if SVO didn't assign good points in the beginning for example.
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    W += (points_new_no_outliers.col(i) - mean_new) * (points_old_no_outliers.col(i) - mean_old).transpose();
  }
  W /= num_points_;

//  Eigen::JacobiSVD<Eigen::Matrix<FloatType,3,3> > svd(
//          W, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::BDCSVD < Eigen::Matrix<FloatType, 3, 3> > svd(W, Eigen::ComputeFullU | Eigen::ComputeFullV);
  ...
  Eigen::Matrix < FloatType, 3, 3 > R = svd.matrixU() * S * svd.matrixV().transpose();
  ...
  {
    double curr_res =
        (points_new_no_outliers.col(i) - R * points_old_no_outliers.col(i) - T_old_new_pre.getPosition()).norm();
    residual += curr_res;
    if (curr_res < 0.2)
    {
      mean_old_no_outliers_newit += points_old_no_outliers.col(i);
      mean_new_no_outliers_newit += points_new_no_outliers.col(i);
      points_new_no_outliers_newit.col(num_points_new) = points_new_no_outliers.col(i);
      points_old_no_outliers_newit.col(num_points_new) = points_old_no_outliers.col(i);
      num_points_new++;
    }
  }
  ...
  //  Eigen::JacobiSVD<Eigen::Matrix<FloatType,3,3> > svd(
  //          W, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::BDCSVD < Eigen::Matrix<FloatType, 3, 3> > svd1(W1, Eigen::ComputeFullU | Eigen::ComputeFullV);

  Eigen::Matrix < FloatType, 3, 3 > S1 = Eigen::Matrix<FloatType, 3, 3>::Identity();
  if (svd1.matrixU().determinant() * svd1.matrixV().determinant() < 0)
  {
    S1(2, 2) = -1;
  }
  Eigen::Matrix < FloatType, 3, 3 > R1 = svd1.matrixU() * S1 * svd1.matrixV().transpose();
  ...
  for (int i = 0; i < num_points_; ++i)
  {
    double curr_res = (points_new_no_outliers_newit.col(i) - R1 * points_old_no_outliers_newit.col(i)
        - T_old_new.getPosition()).norm();
    residual += curr_res;
  }
  ...
  /** Sim3 alignment --------------------------------------------------------
   ...
   Eigen::Matrix<FloatType,3,3> R2 = svd2.matrixU()*S2*svd2.matrixV().transpose();
...
bool MapAlignmentSE3::getTransformRansac(Transformation &T_old_new, const int& min_num_3d, int& n_ransac_inliers, bool& relax_thresh)
{
  ...
  opengv::point_cloud::PointCloudAdapter adapter(points_vec_new_, points_vec_old_);
  ...
  {
    res += (points_vec_new_[ransac.inliers_[i]] - R * points_vec_old_[ransac.inliers_[i]] - transform.col(3)).norm();
  }
...
bool MapAlignmentSE3::solveJointOptimisation(const std::vector<cv::Point3f>& landmarks_lc, const std::vector<cv::Point3f>& landmarks_cf,
                                             ...
                                             const size_t& num_bow_features_cf, const int& min_num_3d)
{
  ...
  int count_3d = -1;
  for (const std::pair<size_t, size_t> &correspondence : keypoint_correspondences)
  {
    if(correspondence.first < num_bow_features_lc && correspondence.second < num_bow_features_cf)
    {
      // 2d match, added as epipolar error
      Eigen::Matrix<double, 2, 1> p_1;
      p_1 << (double)keypoints_lc[correspondence.second].x, (double)keypoints_lc[correspondence.second].y;
      Eigen::Matrix<double, 2, 1> p_2;
      p_2 << (double)keypoints_cf[correspondence.first].x, (double)keypoints_cf[correspondence.first].y;
      ceres::CostFunction* cost_function = ceres_backend::EpipolarError::Create(1.0, p_1, p_2);
  ...
  for (const std::pair<size_t, size_t>& correspondence : point_correspondences)
  {
    ...
    P_w << (double)landmarks_lc[correspondence.first].x, (double)landmarks_lc[correspondence.first].y,
...
Transformation MapAlignmentSE3::getTransformationCombined(Transformation& T_w_lc, Transformation& T_w_cf)
{
  Transformation T_new_old, T_w_new;
  T_w_new = T_w_lc * t_rel_combined_;
  /* Note that this T_new_old transforms the T_cf (current frame or sliding window), to its new location and
